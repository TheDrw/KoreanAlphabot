...
class Settings:
    ...
    # in user_message, it is split into a list,
    # so you'll need to check if it contains more than one element
    ...
    async def set_quiz_mode(cls, bot_command, channel, user_message):
        ...
        mode_help_message = (
            f'<< Current mode is : {cls.quiz_mode} >>\n\n'
        ...
        return await channel.send(
            f'Switching quiz mode to quiz in {quiz_in}. '
            f'You will have to answer in {answer_in}.')


    # the user_message passes a list where [0] is teh command
    # and [1] is the possible time message as strings.
    # if the len is 1, that means there's no time given in the message,
    # so say the default message.
    # if len > 1, that means there's a possible time given,
    # so we check if it is a number and if it is btwn [1,60]
    # i think the regex may be overkill,
    # but i am using it because i am learning regex at the moment
    @classmethod
    async def set_timer(cls, bot_command, channel, user_message):
        MIN_TIMER, MAX_TIMER = 1, 60
        timer_help_message = (
            f'<< Current timer is : {cls.answer_timer} >>\n\n'
            f'To change the mode, type:\n'
        ...
        if not CONTAINS_ONLY_TWO_DIGITS:
            return await channel.send(f'Invalid time!\n{timer_help_message}')
    ...
    async def set_block_size(cls, bot_command, channel, user_message):
        ...
        block_size_help_message = (
            f'<< Current block size is : {cls.block_size} >>\n\n'
        ...
        if not CONTAINS_ONLY_ONE_DIGIT:
            return await channel.send(f'Invalid block size!\n{block_size_help_message}')
    ...
    # see set_timer for implementation details.
    # it is pretty much the same
    @classmethod
    async def set_num_of_words(cls, bot_command, channel, user_message):
        MIN_WORDS, MAX_WORDS = 1, 10
        num_of_words_help_message = (
            f'<< Current number of words: {cls.num_of_words} >>\n\n'
        ...
        if not CONTAINS_ONLY_TWO_DIGITS:
            return await channel.send(f'Invalid number of words!\n{num_of_words_help_message}')
